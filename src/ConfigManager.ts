...
export class ConfigManager {
    ...
    public configSection<T>(path: string, defaultConfig: T): T {
        let root = this.getOrCreateConfigPath(path);
        if (root === defaultConfig) {
            return root;
        }
        this.addDefaultConfig(path, defaultConfig);
        return this.updateConfigPath(path, mergeConfigs(defaultConfig, root || {})) as T;
    }
     ...
     */
    public validateConfig(clearUnusedValues: boolean = false): ConfigValidation {
        const created: Array<string> = [];
        const result: ConfigValidation = {created, unusedValues: [], wrongTypes: []};

        // Resolver function for recursive analyzing
        const resolve = (firstLevel: boolean, path: string, [conf, oldConf, defConf]: [any, any, any]) => {
            ...
            for (let key of Object.keys(conf)) {
                ...
                if (typeof conf[key] === 'object' && typeof defConf[key] === 'object') {
                    ...
                    resolve(false, propKey, [conf[key], oldConf[key] || {}, defConf[key]])
                }
            }
        }
